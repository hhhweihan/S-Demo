...
void *mm_realloc(void *ptr, size_t size)
{
   ...
   /* 如果ptr为NULL，调用等价与mm_malloc(size) */
    ...
    /* 原先的size大小满足，直接返回原来的块 */
    if(GET_SIZE(HDRP(ptr))>=asize){
        return ptr;
    }
    /* 否则先检查ptr的下一个块是否为空闲块，
    如果是空闲块则看看ptr加上下一个块(空闲块)的大小是否满足asize
    满足就可以合并该块。
     */
    else if (!GET_ALLOC(HDRP(NEXT_BLKP(ptr)))
        && (GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(NEXT_BLKP(ptr)))>=asize)){
        total_size = GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(NEXT_BLKP(ptr)));
        _remove(NEXT_BLKP(ptr));
        PUT(HDRP(ptr),PACK(total_size,1));
        PUT(FTRP(ptr),PACK(total_size,1));
    }else if (!GET_ALLOC(HDRP(PREV_BLKP(ptr)))
        && (GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(PREV_BLKP(ptr)))>=asize))
    {
        total_size = GET_SIZE(HDRP(ptr))+GET_SIZE(HDRP(PREV_BLKP(ptr)));
        size_t oldsize = GET_SIZE(HDRP(ptr));
        _remove(PREV_BLKP(ptr));
        PUT(FTRP(ptr),PACK(total_size,1));
        PUT(HDRP(PREV_BLKP(ptr)),PACK(total_size,1));
        newptr = PREV_BLKP(ptr);
        memmove(newptr,ptr,oldsize-2*DSIZE);
    }
    else {
