...
void test_timer_wheel() {
  ...
  expect(expired == std::vector<int>{2}, "TimerWheel expires idle connections and honors refresh");
  wheel.tick();
  expect((expired == std::vector<int>{2, 1}), "TimerWheel expires refreshed connection at the latest tick");
}
...
void test_precision_heap() {
  ...
  heap.add(2, 3);
  heap.add(3, 7);
  expect((heap.pop_expired(7) == std::vector<int>{2, 3}), "PrecisionTimerHeap pops timers by exact deadline");
}
...
void test_io_uring_simulator() {
  ...
  expect(first && first->result == "read:file", "IoUringSimulator produces CQ completion for read");
  expect(second && second->result == "send:socket", "IoUringSimulator produces CQ completion for send");
}
...
void test_optimization_report() {
  ...
  expect(report.recommendation.find(linux_like ? "sendfile" : "WSL2") != std::string::npos, "OptimizationReport reflects platform benchmark limits");
}
