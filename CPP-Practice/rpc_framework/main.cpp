...
void test_codec_frame() {
  ...
  expect(decoded.id == 7 && decoded.payload == "hello|rpc", "RPC request encode/decode preserves escaped payload");
  ...
  expect(codec.next_frame().value_or("") == "payload", "FrameCodec decodes a length-prefixed frame");
}
...
void test_registry_interceptor() {
  ...
  mini_rpc::RpcServer server;
  server.add_interceptor([](const mini_rpc::RpcRequest& request, mini_rpc::RpcResponse& response) {
    if (request.payload == "blocked") { response = {request.id, false, "blocked by interceptor"}; return false; }
    return true;
  });
  server.register_method("Echo", "Say", [](const std::string& payload) { return payload; });
  mini_rpc::RpcClient client(server);
...
void test_simulated_acceptance() {
  ...
  expect(async_qps_proxy > 100000.0, "Simulated async dispatch throughput proxy is above 100k QPS");
