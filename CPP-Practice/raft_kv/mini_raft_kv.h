...
// ============================================================================
// 教学基线（已被 raft_sim.h 取代）—— 上帝视角单进程模型。
//
// 本文件用直接函数调用替代 RPC、由集群对象“俯视”读写每个节点，因此复制是同步且无损的：
// 它无法表现真实 Raft 必须存活的失败——丢包/乱序/重复消息、网络分区、脑裂选主、陈旧 Leader。
// 崩溃恢复也只是整表覆盖 + 文本 WAL，restart() 靠对象保留而非真磁盘重载。
//
// 生产化实现请看同目录的 raft_sim.h：确定性离散事件仿真 + 逐节点真 RPC handler + 虚拟时间网络
// （可丢/延/重/乱/分区）+ 每步安全不变量校验，并在其上补齐 Storage 真持久化、snapshot/
// InstallSnapshot、ReadIndex 线性一致读、Pre-Vote。测试 target（raft_kv_tests）只依赖 raft_sim.h；
// 本文件保留作对照阅读，main.cpp 的 smoke demo 仍用它演示最小选举/复制闭环。
// ============================================================================
