...
void test_leader_election() {
  ...
  expect(cluster.leader_id() == 0 && cluster.node(0).role() == mini_raft::Role::Leader, "Elected node becomes Leader");
}
...
void test_log_replication_kv() {
  ...
  expect(cluster.get("x").value_or("") == "1", "Committed entry applies to leader KV state machine");
  expect(cluster.node(1).store().get("x").value_or("") == "1", "Follower applies committed log entry");
...
void test_persist_snapshot() {
  ...
  expect(recovered.store().get("snap").value_or("") == "value", "Node recovers committed KV state from WAL-like log");

  auto snapshot = recovered.snapshot();
  mini_raft::RaftNode follower(2);
  follower.install_snapshot(snapshot, recovered.commit_index());
  expect(follower.store().get("snap").value_or("") == "value", "Snapshot installs KV state on follower");
}
...
void test_simulated_acceptance() {
  ...
  std::uniform_int_distribution<int> timeout_ms(150, 300);
  std::vector<int> timeouts{timeout_ms(rng), timeout_ms(rng), timeout_ms(rng)};
  auto winner = static_cast<int>(std::min_element(timeouts.begin(), timeouts.end()) - timeouts.begin());
  expect(timeouts[winner] >= 150 && timeouts[winner] <= 300, "Simulated randomized election timeout stays in 150-300ms window");

  mini_raft::RaftCluster cluster(3);
  expect(cluster.elect(winner), "Simulated timeout winner can become leader");
  cluster.crash_leader();
  double failover_ms = elapsed_ms([&] { expect(cluster.elect((winner + 1) % 3), "Simulated failover elects a new leader"); });
  expect(failover_ms < 500.0, "Simulated failover election completes under 500ms wall-clock proxy");
  ...
  double kv_ms = elapsed_ms([&] {
    for (int i = 0; i < operations; ++i) {
      ...
      if (!value || *value != "v" + std::to_string(i)) std::exit(1);
    }
  });
  ...
  expect(recovered.store().get("k4999").value_or("") == "v4999", "LevelDB-like persisted state recovers the latest committed key");
