...
void test_async_logger_multithread() {
    ...
    logger.flush();
    // 核心不变量：并发写入不丢行。accepted 记生产端入队数、written 记后台落盘数，
    // 二者与 sink 实际收到的行数必须三方相等，否则说明队列交接或计数存在竞态。
    expect(logger.accepted() == thread_count * per_thread,
           "AsyncLogger accepts all multi-thread messages");
    expect(logger.written() == logger.accepted() && memory->size() == logger.accepted(),
           "AsyncLogger flushes all messages to sinks");
}
...
void test_multi_sink_file_roll() {
    ...
    logger.flush();
    // 同一条日志要原样广播到每个 sink：文件落盘与控制台缓冲都应能看到它。
    expect(std::filesystem::exists(file->current_path()),
           "FileSink creates a date-stamped log file");
    expect(console_buffer.str().find("multi sink message") != std::string::npos,
           "ConsoleSink receives the same log line");
}
...
void test_enqueue_latency_smoke() {
    ...
    // 只测入队延迟：异步日志的卖点是生产端几乎不阻塞，落盘交给后台线程，
    // 所以计时在入队结束处截止，flush 的等待不计入。
    const auto enqueue_end = std::chrono::steady_clock::now();
    logger.flush();
    const auto enqueue_us =
        std::chrono::duration_cast<std::chrono::microseconds>(enqueue_end - begin).count();
    std::cout << "Async enqueue avg: " << (static_cast<double>(enqueue_us) / messages)
              << " us/message\n";
