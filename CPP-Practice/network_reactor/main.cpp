...
void test_sticky_packet_buffer() {
    ...
    // TCP 是字节流：两帧可能被切成任意片段到达。先只喂 6 字节（不足一整帧），
    // read_frame 必须拒绝，直到长度前缀标出的帧体完整才交付。
...
void test_reactor_core() {
    ...
    loop.update(&channel);
    loop.activate(10);
    expect(loop.loop_once() == 1 && reads == 1,
           "EventLoop dispatches active Channel read callbacks");

    // one-loop-per-thread：线程池按轮转把连接分给不同 loop，同一 loop 内单线程无锁。
    mini_net::EventLoopThreadPool pool(2);
    auto& first = pool.next_loop();
    auto& second = pool.next_loop();
    expect(&first != &second && pool.size() == 2,
           "EventLoopThreadPool rotates loops for one-loop-per-thread design");
}

void test_http_core() {
    mini_net::HttpServerCore server;
    server.get("/hello",
               [](const mini_net::HttpRequest& request) { return "hello " + request.path; });
    ...
    expect(response.find("hello /hello") != std::string::npos,
           "HTTP response contains handler body");
}
...
void test_rpc_core() {
    ...
    expect(client.call("echo", "payload") == "OK:payload",
           "RPC core completes one request-response round trip");
}
