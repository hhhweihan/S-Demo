...
// do/while(false) 包裹：让宏在 `if (x) EXPECT_TRUE(y); else ...` 中仍是单条语句。
...
void test_day51_countdown_latch() {
    ...
    // 5ms 短超时应在 worker 睡够之前返回 false，验证 wait_for 的超时分支。
...
BenchmarkResult run_thread_pool_benchmark(std::size_t worker_count) {
    ...
    return BenchmarkResult{"thread_pool_" + std::to_string(worker_count) + "_threads", elapsed_ms,
...
BenchmarkResult run_single_thread_benchmark() {
    ...
    EXPECT_EQ(counter.load(std::memory_order_relaxed), kBenchmarkTaskCount);
    return BenchmarkResult{"single_thread_baseline", elapsed_ms,
                           kBenchmarkTaskCount / (elapsed_ms / 1000.0)};
}
...
void test_day52_pause_resume_and_await_termination() {
    ...
    // 独立线程提交第二批：暂停期间它应阻塞在 submit 里，一个都进不去。
    ...
    EXPECT_TRUE(paused_stats.accepting);  // 暂停 != 停止接受，resume 后仍可提交
...
BenchmarkResult run_blocking_queue_benchmark() {
    ...
    const double elapsed_ms = measure_ms([&] {
        ...
        std::thread consumer([&] {
            for (int expected = 0; expected < kQueueBenchmarkCount; ++expected) {
                ...
                EXPECT_EQ(value, expected);  // 单生产单消费下队列保持 FIFO
            }
        });
    ...
    return BenchmarkResult{"blocking_queue_1p1c", elapsed_ms,
...
LatencyBenchmarkResult run_thread_pool_latency_benchmark(std::size_t worker_count,
    ...
    // 测提交->开始执行的排队延迟：任务捕获提交时刻，进入执行时再算差值。
    ...
    pool.await_termination();
    std::sort(latencies.begin(), latencies.end());  // percentile_us 依赖有序输入

    return LatencyBenchmarkResult{percentile_us(latencies, 0.50), percentile_us(latencies, 0.99)};
}
...
int main() {
    try {
        ...
        std::cout << "  " << pool4.name << ": " << pool4.elapsed_ms << " ms, QPS=" << pool4.qps
                  << "\n";
        std::cout << "  " << pool8.name << ": " << pool8.elapsed_ms << " ms, QPS=" << pool8.qps
                  << "\n";
        return 0;
