...
void test_day44_lock_free_stack_parallel_push() {
    ...
    while (popped_count < total_values) {
        ...
        EXPECT_TRUE(seen[static_cast<std::size_t>(value)] == 0);
    ...
    for (int consumer_id = 0; consumer_id < kStackStressConsumers; ++consumer_id) {
        consumers.emplace_back([&] {
            while (true) {
                ...
                if (seen[static_cast<std::size_t>(index)].fetch_add(1, std::memory_order_relaxed) != 0) {
                    valid.store(false, std::memory_order_relaxed);
                }
...
void test_day46_aba_defense() {
    ...
    EXPECT_TRUE(result.plain_head_cas_succeeded);
    EXPECT_TRUE(!result.tagged_head_cas_succeeded);
    EXPECT_EQ(result.final_plain_value, static_cast<std::uint32_t>(3));
    EXPECT_EQ(result.final_tagged_index, static_cast<std::uint32_t>(1));
    EXPECT_EQ(result.final_tagged_tag, static_cast<std::uint32_t>(2));
}
...
BenchmarkResult run_std_queue_baseline() {
    ...
    return BenchmarkResult{"std::queue_baseline",
                           elapsed_ms,
...
BenchmarkResult run_blocking_queue_benchmark() {
    ...
    EXPECT_TRUE(order_ok.load(std::memory_order_relaxed));
    return BenchmarkResult{"BlockingQueue_1P1C",
                           elapsed_ms,
                           kExchangeCount / (elapsed_ms / 1000.0)};
}
...
BenchmarkResult run_bounded_queue_benchmark() {
    ...
    EXPECT_TRUE(order_ok.load(std::memory_order_relaxed));
    return BenchmarkResult{"BoundedQueue_1P1C_cap1024",
                           elapsed_ms,
                           kExchangeCount / (elapsed_ms / 1000.0)};
}
...
BenchmarkResult run_spsc_benchmark() {
    ...
    EXPECT_TRUE(order_ok.load(std::memory_order_relaxed));
    EXPECT_TRUE(queue.empty());
    return BenchmarkResult{"SPSCQueue_1P1C_cap1024",
                           elapsed_ms,
                           kExchangeCount / (elapsed_ms / 1000.0)};
}
...
BenchmarkResult run_lock_free_stack_push_drain_benchmark() {
    ...
    return BenchmarkResult{"LockFreeStack_4P1C_push_drain",
                           elapsed_ms,
...
BenchmarkResult run_reclaiming_stack_push_drain_benchmark() {
    ...
    return BenchmarkResult{"ReclaimStack_4P1C_push_drain",
                           elapsed_ms,
                           total_values / (elapsed_ms / 1000.0)};
}
...
BenchmarkResult run_tagged_pointer_stack_push_drain_benchmark() {
    ...
    return BenchmarkResult{"TaggedStack_4P1C_push_drain",
                           elapsed_ms,
                           total_values / (elapsed_ms / 1000.0)};
}
...
BenchmarkResult run_reclaiming_stack_mpmc_benchmark() {
    ...
    EXPECT_EQ(consumed.load(std::memory_order_acquire), total_values);
    return BenchmarkResult{"ReclaimStack_4P4C_push_pop",
                           elapsed_ms,
                           total_values / (elapsed_ms / 1000.0)};
}
...
void print_benchmark(const BenchmarkResult& result) {
    std::cout << "  " << std::setw(28) << std::left << result.name << " "
              << std::setw(10) << std::right << result.elapsed_ms << " ms"
...
int main() {
    try {
        ...
        const BenchmarkResult lock_free_stack_push_drain = run_lock_free_stack_push_drain_benchmark();
        const BenchmarkResult tagged_stack_push_drain = run_tagged_pointer_stack_push_drain_benchmark();
        const BenchmarkResult reclaim_stack_push_drain = run_reclaiming_stack_push_drain_benchmark();
        ...
        std::cout << "Stack benchmark (" << (kStackStressProducers * kStackBenchmarkValuesPerProducer)
              << " values)\n";
